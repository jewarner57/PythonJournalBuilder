...
def getWeekdays(startDate):
    ...
    while len(weekdays) < 5:
        weekdays.append(
            correctInvalidDates(
                (
                    weekdays[counter - 1].year,
                    weekdays[counter - 1].month,
                    weekdays[counter - 1].day + 1,
                )
            )
        )
        counter += 1
...
def writeFile(journalDays):
    ...
    while counter < 5:
        ...
        newFile.write(
            "\n### "
            + str(calendar.day_name[journalDays[counter].weekday()])
            + " "
            + str(journalDays[counter].month)
            + "/"
            + str(journalDays[counter].day)
        )
        newFile.write("\n* * *")
